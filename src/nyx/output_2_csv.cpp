...
#define fopen_s(pFile,filename,mode) ((*(pFile))=fopen((filename),(mode)))==NULL
		...
		const std::string & mfpath)
	{
		...
		textcols.push_back (u8string_to_string((fs::path(ifpath)).filename().u8string()));
		textcols.push_back ("");
		...
		const Dataset & dataset)
	{
		...
		for (const auto l : L)
		{
			...
			filenames.push_back (u8string_to_string(pint.filename().u8string()));
			filenames.push_back (u8string_to_string(pseg.filename().u8string()));
