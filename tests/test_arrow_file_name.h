...
void test_file_naming () {
    ...
    if (num_deleted == 0) {
        std::cerr << "WARNING: Temporary directory at " << Nyxus::u8string_to_string(temp.u8string()) << " could not be deleted." << std::endl;
    }
}
...
void test_file_naming_case_1 (const fs::path& temp_dir) {
    ...
    auto result = get_arrow_filename(Nyxus::u8string_to_string(output_path.u8string()), "NyxusFeatures", SaveOption::saveArrowIPC);

    std::string expected = Nyxus::u8string_to_string(temp_dir.u8string()) + "foo1/bar1/NyxusFeatures.arrow";
...
void test_file_naming_case_2 (const fs::path& temp_dir) {
    ...
    std::string expected = Nyxus::u8string_to_string(temp_dir.u8string()) + "foo2/bar2.arrow";
...
void test_file_naming_case_3 (const fs::path& temp_dir) {
    ...
    std::string expected = Nyxus::u8string_to_string(temp_dir.u8string()) + "foo3/bar3/NyxusFeatures.arrow";
...
void test_file_naming_case_4 (const fs::path& temp_dir) {
    ...
    std::string expected = Nyxus::u8string_to_string(temp_dir.u8string()) + "foo4/bar4.ext/NyxusFeatures.arrow";
...
void test_file_naming_case_5 (const fs::path& temp_dir) {
    ...
    std::string expected = Nyxus::u8string_to_string(temp_dir.u8string()) + "foo4/bar4.arrow";
